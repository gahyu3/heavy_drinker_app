...
RSpec.describe 'Ranks', type: :system do
  ...
  let(:other_drink_beer) do
    create(:drink, name: '中瓶', volume: 500, degree: 5, user: other_user, category: category_beer)
  end
  let(:record_today) { create(:record, date: Date.today, quantity: 5, user:, drink: drink_beer) }
  let(:record_yesterday) { create(:record, date: Date.yesterday, quantity: 5, user:, drink: drink_beer) }
  let(:record_last_week) { create(:record, date: Date.today - 7, quantity: 5, user:, drink: drink_beer) }
